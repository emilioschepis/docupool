...
serve(async (req) => {
  ...
  const { data: coins, error: coinsError } = await adminClient
    .from("user_coins")
    .select("*")
    .eq("user_id", document.profile.id)
    .single();

  if (coinsError) {
    console.warn(coinsError);
    return new Response("error", {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  ...
  await adminClient
    .from("user_coins")
    .update({ coins: coins.coins + 1 })
    .match({ user_id: document.profile.id })
    .single();
